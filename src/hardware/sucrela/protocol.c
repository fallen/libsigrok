...
static int sucrela_start_sampling(const struct sr_dev_inst *sdi)
{
	...
	uartbone_write(ctx, CSR_LA_SUBSAMPLER_VALUE_ADDR, subsampler);
...
static void LIBUSB_CALL receive_transfer(struct libusb_transfer *transfer)
{
	...
	if (!oversampling)
		...
		if (oversampling_ratio == 2) {
			for (unsigned int i = 0; i < transfer->actual_length;
			     i++) {
				buffer[2 * i] = transfer->buffer[i] & 0x0f;
				buffer[2 * i + 1] =
					(transfer->buffer[i] & 0xf0) >> 4;
			}
		}
		if (oversampling_ratio == 4) {
			for (unsigned int i = 0; i < transfer->actual_length;
				...
				buffer[4 * i + 1] =
					(transfer->buffer[i] & 0x0c) >> 2;
				buffer[4 * i + 2] =
					(transfer->buffer[i] & 0x30) >> 4;
				buffer[4 * i + 3] =
					(transfer->buffer[i] & 0xc0) >> 6;
			}
		}
	}
	...
	if (devc->trigger_fired) {
		...
		trigger_offset = soft_trigger_logic_check(
			devc->stl, buffer + processed_samples * unitsize,
	...
	if (frame_ended) {
		...
		resubmit_transfer(
			transfer); // we still need to process USB until capture is really stopped
	} else {
		sr_err("resubmit !\n");
		resubmit_transfer(transfer);
	}
...
SR_PRIV int dev_acquisition_start(const struct sr_dev_inst *sdi)
{
	...
	usb_source_add(sdi->session, drvc->sr_ctx, 5000, receive_data,
		       (void *)sdi);
...
SR_PRIV int sucrela_dev_open(struct sr_dev_inst *sdi, struct sr_dev_driver *di)
{
	...
	devc->max_samplerate =
		uartbone_read(devc->uartbone_ctx, CSR_LA_SAMPLERATE_ADDR);
	hspi_width = uartbone_read(devc->uartbone_ctx,
				   CSR_LA_HSPI_TX_HSPI_WIDTH_R_ADDR);
	devc->probe_number =
		uartbone_read(devc->uartbone_ctx, CSR_LA_NUM_PROBES_ADDR);
	devc->oversampler_phy_ratio = uartbone_read(
		devc->uartbone_ctx, CSR_LA_OVERSAMPLER_PHY_RATIO_ADDR);
	...
	do {
		c = uartbone_read(devc->uartbone_ctx,
				  CSR_IDENTIFIER_MEM_BASE + i * 4);
		ident_str[i++] = c;
	} while (c);
	sr_err("ident: %s\n", ident_str);
